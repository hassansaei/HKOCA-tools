...
def generate_summary_plots(input_file: str,
    ...
    # bit of pre-compute
    cells = np.asarray(adata.obs['cell_probability'] > consts.CELL_PROB_CUTOFF)
...
def assess_overall_count_removal(adata, raw_full_adata, input_layer_key='raw', out_key='cellbender'):
    global warnings
    cells = np.asarray(adata.obs['cell_probability'] > 0.5)
...
def plot_counts_and_probs_per_cell(adata, input_layer_key='raw'):
    ...
    if limit_to_features_analyzed:
        var_logic = np.asarray(adata.var['cellbender_analyzed'])
    else:
        var_logic = ...
...
def plot_gene_expression_pca(adata, key='cellbender_embedding',
    ...
    if extended:
        ...
        def _pca_color(g: int, layer):
            outcounts = np.array(adata.layers['cellbender'][:, np.asarray(adata.var_names == g)].todense()).squeeze()
            rawcounts = np.array(adata.layers[input_layer_key][:, np.asarray(adata.var_names == g)].todense()).squeeze()
...
def cluster_and_compare_expression_to_truth(adata, embedding_key='cellbender_embedding'):
    ...
    for k in np.unique(adata.obs['cluster'][cells]):
        _ctype = np.asarray(adata.obs['cluster'] == k)
        plt.plot(adata.obsm['X_pca'][_ctype, 0],
                 adata.obsm['X_pca'][_ctype, 1],
                 '.', ls='', ms=5, alpha=0.25)
    ...
    for k in adata.obs['cluster'].unique():
        ...
        summmed_expression = np.array(
            adata.layers['cellbender'][np.asarray(adata.obs['cluster'] == k), :]
            .sum(axis=0)
        ).squeeze()
...
def compare_latents(adata, input_layer_key='raw'):
    ...
    if truth_exists:
        cells = np.asarray(adata.obs['truth_cell_probability'] == 1)
        yy, _ = np.histogram(
