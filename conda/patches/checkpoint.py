...
def _torch_load_with_dill(path, **load_kwargs):
    """Load a torch checkpoint with dill (compatible with PyTorch 2.x)."""
...
def load_from_checkpoint(filebase: Optional[str],
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if 'model' in to_load:
            model_obj = _torch_load_with_dill(filebase + '_model.torch', **load_kwargs)
            logger.debug('Model loaded from ' + filebase + '_model.torch')
            out.update({'model': model_obj})

        # Load the saved optimizer.
        if 'optim' in to_load:
            scheduler = _torch_load_with_dill(filebase + '_optim.torch', **load_kwargs)
        ...
        if 'dataloader' in to_load:
            ...
            if os.path.exists(filebase + '_train.loaderstate'):
                train_loader = _torch_load_with_dill(filebase + '_train.loaderstate', **load_kwargs)
            ...
            if os.path.exists(filebase + '_test.loaderstate'):
                test_loader = _torch_load_with_dill(filebase + '_test.loaderstate', **load_kwargs)
