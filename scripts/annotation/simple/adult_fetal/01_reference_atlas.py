...
def merge_pan_endothelial(out_adult_dir, endo_files, top_n=20):
    ...
    try:
        ...
        if dfs:
            ...
            top_endo["group"] = "adult_endothelial_cell"
            ...
            output_path = out_adult_dir / "adult_endothelial_cell.tsv"
            top_endo.to_csv(output_path, sep="\t", index=False)
            logging.info(f"Generated 'adult_endothelial_cell.tsv' with {len(top_endo)} markers.")
        else:
...
def generate_deliverables(adult_markers_raw, fetal_markers_raw, adult_canonical_map, fetal_canonical_map, dirs, config):
    ...
    try:
        ...
        overlap_records = []
        jaccard_matrix = pd.DataFrame(index=sorted(list(adult_canons)), columns=sorted(list(fetal_canons)), dtype=float)
        ...
        for a_canon in sorted(list(adult_canons)):
            ...
            for f_canon in sorted(list(fetal_canons)):
                ...
                union = a_genes.union(f_genes)
                jaccard = len(intersect) / len(union) if union else 0.0
                
                jaccard_matrix.loc[a_canon, f_canon] = jaccard
        ...
        # 6. Marker Overlap Figure
        if config["plotting"]["save_figs"]:
            plt.figure(figsize=(12, 10))
            jaccard_matrix = jaccard_matrix.fillna(0.0).astype(float)
            count_matrix = count_matrix.fillna(0).astype(int)

            sns.heatmap(jaccard_matrix, annot=count_matrix, fmt="d", cmap="YlGnBu", cbar_kws={'label': 'Jaccard Similarity Score'})
            plt.title(f"Cross-Atlas Marker Overlap Across Top {params['overlap_n']} Genes\n(Colors: Jaccard Score | Text: Shared Gene Count)", fontsize=14, pad=20)
            plt.xlabel("Fetal Cell Types (Canonical)", fontsize=12)
            plt.ylabel("Adult Cell Types (Canonical)", fontsize=12)
...
def main():
    ...
    if config.get("run_diagnostics", False):
        check_threshold_elbow(
            ...
            params=config["elbow_params"]
        )
        check_threshold_elbow(
            fetal_markers_raw, 
            cell_type="Distal_Convoluted_Tubule", 
    ...
    export_final_markers_to_tsv(
        ...
        "adult", adult_canonical_map, config["elbow_params"]
    )
    export_final_markers_to_tsv(
        fetal_markers_raw, dirs["out_fetal"], config["elbow_params"]["fetal_tolerance"], 
        "fetal", fetal_canonical_map, config["elbow_params"]
    )

    # Post-Processing: Pan-Endothelial Merge
    merge_pan_endothelial(dirs["out_adult"], config["pan_endothelial"]["files_to_merge"], config["pan_endothelial"]["top_n"])
